...
tstamp = datetime.datetime.now(datetime.timezone.utc).strftime("%Y-%m-%d-%H_%M")
...
def _extract_single_zipitem(
    zip_path, info_item, output_path, skip_existing: bool = True
):
    try:
...
def extractFromZip(
    zip_path: Path | str, output_path: Path | str, skip_existing: bool = True
):
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=4) as executor:
        futures = [
            executor.submit(
                _extract_single_zipitem, zip_path, info_item, output_path, skip_existing
            )
            for info_item in infolist
        ]
        for future in concurrent.futures.as_completed(futures):
            if future.exception():
                logging.error(f"Error extracting zipfile: {future.exception()}")
...
def _convert_single_csv_to_parquet(
    ...
    try:
        parquet_file = parquet_path.joinpath(
            csv_file.parent.name, csv_file.stem + ".parquet"
        )
...
def convertCsvToParquet(
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=4) as executor:
        futures = [
            executor.submit(
                _convert_single_csv_to_parquet,
                csv_file,
                parquet_path,
...
class Csv2ParquetPipeline:
    ...
    def extract_csvs_from_zip(self):
        return extractFromZip(
            self.zip_path, self.csv_path, skip_existing=self.skip_existing
        )

    def convert_csvs_to_parquet(self):
        return convertCsvToParquet(
            self.csv_path, self.parquet_path, skip_existing=self.skip_existing
        )
